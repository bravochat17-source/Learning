...
function toggleFolder(key){if(S.opened&&key===S.lF)closeFolder();else openFolder(key);}
function initFanSwipe(){
  const ar=document.querySelector('.archive');if(!ar)return;
  const down=(x,y,target)=>{const fo=target&&target.closest?target.closest('.folder'):null;fanDrag={x,y,start:selIndex(),moved:0,axis:null,key:fo?fo.getAttribute('data-k'):null};ar.classList.add('dragging');};
  const move=(x,y,e)=>{if(!fanDrag)return;const dx=x-fanDrag.x,dy=y-fanDrag.y;
    if(!fanDrag.axis&&(Math.abs(dx)>7||Math.abs(dy)>7))fanDrag.axis=Math.abs(dx)>Math.abs(dy)?'x':'y';
    if(fanDrag.axis==='y')return;
    if(fanDrag.axis==='x'&&e&&e.cancelable)e.preventDefault();
    fanDrag.moved=Math.max(fanDrag.moved,Math.abs(dx));let p=fanDrag.start-dx/FAN_STEP;p=Math.max(0,Math.min(CATS.length-1,p));fanPos=p;fanQueuedPos=p;if(!fanFrame)fanFrame=nextFrame(()=>{fanFrame=0;if(fanQueuedPos!==null){positionFolders(fanQueuedPos,false);fanQueuedPos=null;}});
  };
  const up=(x,target)=>{if(!fanDrag)return;const axis=fanDrag.axis,dist=fanDrag.moved;
    if(axis==='y'){ar.classList.remove('dragging');positionFolders(selIndex(),true);fanDrag=null;return;}
    if(dist<12){const fo=target&&target.closest?target.closest('.folder'):null,k=fanDrag.key||(fo?fo.getAttribute('data-k'):null);if(k)toggleFolder(k);else positionFolders(selIndex(),true);}
    else{if(S.opened)closeFolder();let ns=Math.max(0,Math.min(CATS.length-1,Math.round(fanPos)));setCat(CATS[ns].key);}ar.classList.remove('dragging');fanQueuedPos=null;fanDrag=null;
  };
  ...
  ar.addEventListener('touchend',e=>up(e.changedTouches[0].clientX,e.target),{passive:true});
  ...
  window.addEventListener('mouseup',e=>{if(Date.now()-lastFanTouch<700)return;if(fanDrag)up(e.clientX,e.target);});
...
function renderList(){
  ...
  el.innerHTML=list.map(w=>{const tot=w.correct+w.wrong,pct=tot?Math.round(w.correct/tot*100):0;const dots=Array.from({length:5},(_,i)=>`<div class="md${i<Math.round(pct/20)?' on':''}"></div>`).join('');const sc=w.status==='unknown'?'<span class="badge bs" style="margin-bottom:2px">不熟</span>':w.status==='known'?'<span class="badge bw" style="margin-bottom:2px">已記住</span>':'';return`<div class="wrow" onclick="openD('${w.id}')"><div style="flex:1;min-width:0"><div class="wen">${esc(w.term)}</div><div class="wzh">${esc(w.meaning)}</div>${w.note?`<div class="wnote">${esc(w.note)}</div>`:''}<div class="mdots">${dots}</div></div><button class="wspk" onclick="event.stopPropagation();speakWord('${w.id}','term',this)" aria-label="發音">${SPK_SVG}</button><div class="rt">${sc}<span class="badge ${bC(w.type)}">${bL(w.type)}</span><span class="chev">›</span></div></div>`;}).join('');
}
...
function attachCardGestures(card){
  let sx=0,sy=0,drag=false,moved=false,axis=null,cardFrame=0,pendingDx=0;
  ...
  function start(x,y){if(card.dataset.gone)return;sx=x;sy=y;drag=true;moved=false;axis=null;card.classList.add('dragging');card.style.transition='none';}
  function move(x,y,e){if(!drag)return;const dx=x-sx,dy=y-sy,ax=Math.abs(dx),ay=Math.abs(dy);if(e&&e.cancelable)e.preventDefault();if(!axis&&(ax>4||ay>4)){if(ax>=ay*.65)axis='x';else if(ay>ax*1.5)axis='y';}if(axis==='y')return;if(axis==='x'){moved=true;pendingDx=dx;if(!cardFrame)cardFrame=nextFrame(()=>{cardFrame=0;const d=pendingDx;card.style.transform=`translate3d(${d}px,0,0) rotate(${d/24}deg)`;card.style.opacity=String(Math.max(0.5,1-Math.abs(d)/700));showStamp(d);});}}
  function end(x,y){if(!drag)return;drag=false;card.classList.remove('dragging');const dx=x-sx,dy=y-sy;
    if(axis==='y'){card.style.transition='transform .3s,opacity .3s';card.style.transform='';card.style.opacity='';clrStamp();return;}
    if(Math.abs(dx)>70){cAns(dx>0);return;}
