...
function folderPaperCount(count){if(count<=0)return 0;return Math.max(1,Math.min(3,Math.ceil(count/Math.max(1,S.words.length)*3)));}
function folderPapers(count,ti){const n=folderPaperCount(count),s3=`<div class="sheet s3"${ti}><div class="ln"></div><div class="ln"></div><div class="ln"></div><div class="ln"></div><div class="ln"></div></div>`;if(n===0)return'';if(n===1)return s3;if(n===2)return`<div class="sheet s2"${ti}></div>${s3}`;return`<div class="sheet s1"${ti}></div><div class="sheet s2"${ti}></div>${s3}`;}
function renderFan(){
  ...
  fan.innerHTML=CATS.map((c,i)=>{const cnt=catCount(c.key);const ti=(t=>t?` style="${t}"`:'')(catTint(c.key));return `<div class="folder" data-i="${i}" data-k="${c.key}" role="button" tabindex="0" aria-label="${c.zh}，${cnt} 個詞彙">
      ...
      <div class="docs">${folderPapers(cnt,ti)}</div>
      <div class="pocket">
...
/* ---- detail overlay ---- */
function openD(id){const w=S.words.find(x=>x.id===id);if(!w)return;const tot=w.correct+w.wrong,pct=tot?Math.round(w.correct/tot*100):0;document.getElementById('dc').innerHTML=`<div style="margin-bottom:8px"><span class="badge ${bC(w.type)}">${bL(w.type)}</span></div><div class="d-speak-row d-term-row"><div class="d-term">${esc(w.term)}</div><button class="spk dspk" onclick="speakWord('${w.id}','term',this)" aria-label="播放單字發音">${SPK_SVG}</button></div><div class="d-zh">${esc(w.meaning)}</div>${w.example?`<div class="d-speak-row d-example-row"><div class="d-ex">${esc(w.example)}</div><button class="spk dspk d-example-spk" onclick="speakWord('${w.id}','example',this)" aria-label="播放例句發音">${SPK_SVG}</button></div>`:''}${w.note?`<div class="d-note">${esc(w.note)}</div>`:''}<div class="slbl" style="margin-bottom:8px">掌握度 · Mastery</div><div style="display:flex;align-items:center;gap:11px;margin-bottom:14px"><div class="d-mtrack"><div class="d-mfill" style="width:${pct}%"></div></div><span style="font-family:var(--mono);font-size:calc(var(--fs)*0.82);color:var(--ink3);min-width:38px;text-align:right">${tot?pct+'%':'—'}</span></div><p style="font-family:var(--mono);font-size:calc(var(--fs)*0.78);color:var(--ink3);margin-bottom:20px">${tot?`答對 ${w.correct} · 答錯 ${w.wrong}`:'尚未測驗'}</p><button class="del-btn" onclick="delW('${w.id}')">刪除此詞彙</button>`;document.getElementById('ov').classList.add('show');document.documentElement.classList.add('modal-open');}
...
function clearCurrentLibrary(){
  const count=S.words.length,fb=document.getElementById('exp-fb');
  if(!count){if(fb)fb.innerHTML='<div class="fb-r">目前字庫已經是空的。</div>';return;}
  uiConfirm(`確定要刪除目前字庫的 ${count} 個詞彙嗎？所有熟悉度、答題與測驗紀錄也會一併刪除。此操作無法復原，建議先匯出完整 JSON 備份。`,'刪除全部',function(){
    S.words=[];S.doneToday=[];S.cards=null;S.quiz=null;S.quizLog={};S.opened=false;S.lF='all';S.cF='all';S.qF='all';S.cardDay=todayStr();S.startDate=todayStr();listLimit=LIST_BATCH;
    save();updH();renderFan();renderList();renderCalendar();updateQToday();renderCSetup();
    const ec=document.getElementById('exp-count');if(ec)ec.textContent='0';
    if(fb)fb.innerHTML='<div class="fb-g">✓ 目前字庫與學習紀錄已刪除。</div>';
    if(cloudSession)setTimeout(()=>cloudPut(packState()).then(()=>cloudSetStatus('空字庫已同步至雲端','ok')).catch(e=>cloudSetStatus(cloudErrorMessage(e),'err')),80);
  });
}
